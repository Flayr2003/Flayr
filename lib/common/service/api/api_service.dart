...
import 'package:flayr/common/service/utils/params.dart';
import 'package:flayr/screen/session_expired_screen/session_expired_screen.dart';
    ...
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        try {
          ...
          Loggers.error("Failed to decode JSON: ${response.body}");
        ...
        if (decodedResponse['status'] == false) {
          Loggers.error('API ERROR RESPONSE: ${decodedResponse['message']}');
          // If the message is about missing fields, we should know
          if (onError != null) {
            onError();
          } else {
            // Show snackbar for API level errors if no custom error handler
            BaseController.share.showSnackBar(decodedResponse['message']?.toString());
          }
        }

        var prettyString = const JsonEncoder.withIndent('  ').convert(decodedResponse);
        Loggers.info("API SUCCESS RESPONSE: $prettyString");
